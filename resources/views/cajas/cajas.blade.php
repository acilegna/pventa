...
<div class="container-fluid">
    ...
    <form action="{{url('newcaja')}}" method="POST" id="regForm">
        ...
        <div class="input-group">
            <input type="text" class="form-control{{ $errors->has('inputCaja') ? ' is-invalid' : '' }}" placeholder="Nombre de la caja" aria-label="Text input with dropdown button" name="inputCaja">
            @if ($errors->has('inputCaja'))
                <span class="invalid-feedback" role="alert">
                <strong>{{ $errors->first('inputCaja') }}</strong>
                </span>
            @endif
            <select class="custom-select form-control{{ $errors->has('inputStatus') ? ' is-invalid' : '' }}" id="inputStatus" name="inputStatus">
            ...
            <div class="btn-group fl-rigth">  
                <button class="btn btn-success" type="submit" name="btn2" value="alta"><i class="fa fa-cube"></i> Guardar</button>
            </div>  
            <div class="btn-group fl-rigth ">
